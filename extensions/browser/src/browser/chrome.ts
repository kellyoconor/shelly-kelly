...
export async function launchOpenClawChrome(
  ...
  // Check for BROWSER_WS_ENDPOINT first - if it exists, use remote CDP connection
  if (process.env.BROWSER_WS_ENDPOINT) {
    log.info(`🦞 using remote browser via BROWSER_WS_ENDPOINT: ${process.env.BROWSER_WS_ENDPOINT}`);

    try {
      // Test the connection to make sure it works
      const browser = await chromium.connectOverCDP(process.env.BROWSER_WS_ENDPOINT, {
        timeout: resolved.remoteCdpTimeoutMs || 8000,
      });

      // Close the test connection immediately
      await browser.close();

      // Create a mock process since RunningChrome requires one
      // This is a workaround - ideally the type should support remote connections
      const mockProc = {
        pid: -1,
        kill: () => true,
        exitCode: null,
        ...
        emit: () => false,
        off: () => mockProc,
        removeListener: () => mockProc,
        removeAllListeners: () => mockProc,
        setMaxListeners: () => mockProc,
        getMaxListeners: () => 0,
        listeners: () => [],
        rawListeners: () => [],
        listenerCount: () => 0,
        prependListener: () => mockProc,
        prependOnceListener: () => mockProc,
        eventNames: () => [],
      ...
      return {
        pid: -1, // No real process ID for remote connection
        exe: {
          kind: "remote-browserless" as unknown as BrowserExecutable["kind"],
          path: process.env.BROWSER_WS_ENDPOINT,
        },
        userDataDir: "/tmp/remote-browser", // Mock user data dir
        cdpPort: profile.cdpPort, // Keep the configured port for compatibility
        startedAt,
        proc: mockProc,
      };
    } catch (error) {
      throw new Error(
        `Failed to connect to remote browser at ${process.env.BROWSER_WS_ENDPOINT}: ${String(error)}`,
        { cause: error },
      );
    }
  }

  // Existing local Chrome launch logic continues here...
