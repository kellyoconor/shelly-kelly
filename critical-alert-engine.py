...
Kelly's Critical Alert System
    ...
    def mark_response_received(self, alert_id: str = None, confirmed_by_user: bool = True, minutes_back: int = None):
        """Mark specific alert as received - ONLY when Kelly explicitly responds"""
        alerts = self.load_alerts()
        marked_count = 0
        
        if minutes_back is not None:
            # Time-based marking for auto-confirmation when Kelly is active
            import datetime
            ...
            for alert in alerts:
                ...
                if alert_time >= cutoff_time:
                    ...
                    alert["status"] = "AUTO_CONFIRMED"
                    alert["resolved"] = True
                    marked_count += 1
                    self.log(f"Auto-marked alert {alert['id']} as responded (within {minutes_back} minutes)")
        
        elif alert_id:
            # Specific alert marking
            for alert in alerts:
                if alert["resolved"]:
                    continue
                
                if alert["id"] == alert_id and confirmed_by_user:
                    alert["response_received"] = True
                    alert["status"] = "CONFIRMED"
                    alert["resolved"] = True
                    marked_count += 1
                    self.log(f"Alert {alert_id} confirmed by Kelly's explicit response")
                    break  # Only mark the specific alert
    ...
    def validate_delivery_with_user_response(self, alert_id: str, user_message: str) -> bool:
        """Validate alert delivery by checking if user specifically acknowledges the alert"""
        alerts = self.load_alerts()
        alert = next((a for a in alerts if a["id"] == alert_id), None)
        ...
        # Check if user message references the alert content or acknowledges receipt
        acknowledgment_patterns = [
            alert_id,  # Alert ID mentioned
            "got it", "received", "confirmed", "yes", "okay", "ok", 
            "test received",  # For test alerts
            "thanks", "thank you"
        ]
        
        user_msg_lower = user_message.lower()
        
        # For health alerts, check for health-related responses
        if alert["category"] == "health":
            health_responses = [
                "feeling", "health", "doctor", "better", "worse", "pain", 
                "tired", "energy", "sleep", "fine", "good", "bad"
            ]
            acknowledgment_patterns.extend(health_responses)
        
        # Check if user message contains acknowledgment
        if any(pattern in user_msg_lower for pattern in acknowledgment_patterns):
            # Mark as truly delivered
            alert["response_received"] = True
            alert["status"] = "CONFIRMED"
            alert["resolved"] = True
            
            # Add confirmation attempt record
            confirmation = {
                "timestamp": datetime.utcnow().isoformat(),
                "method": "user_response",
                "success": True,
                "user_message": user_message[:100],  # Store snippet for verification
                "confirmed": True
            }
            alert["delivery_attempts"].append(confirmation)
            
            self.save_alerts(alerts)
            self.log(f"Alert {alert_id} TRULY confirmed by user response: '{user_message[:50]}...'")
            return True
        
        return False

    def check_unconfirmed_alerts(self) -> List[Dict]:
        """Get all unconfirmed alerts that actually failed delivery"""
        alerts = self.load_alerts()
        unconfirmed = []
