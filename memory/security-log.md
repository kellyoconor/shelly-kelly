...
- Security review passed — all clear
\n2026-06-08T06:00:20.[REDACTED_CLIENT_ID]: Auto-redacted 8 exposed credentials from files\n\n2026-06-08T06:00:53.[REDACTED_CLIENT_ID]: Auto-redacted 1 exposed credentials from files\n\n## 2026-06-08 — Nightly Security Review (02:00 America/New_York)\n- Auto-redaction ran first and removed 1 exposed credential from `memory/security-log.md`; exposure was auto-remediated and not escalated per policy\n- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live secret exposures after redaction; remaining hits were prior redacted security-log notes, one learning-log mention, dependency text, and one Playwright prompt artifact\n- Focused hardcoded-credential scan across `/data/workspace` found no live hardcoded credentials; remaining matches were documentation/examples and one Slack placeholder (`xoxb-...`) in `src/server.js`, not secrets\n- Git review (last 24h): expected commit activity only — `[REDACTED_CLIENT_ID]e4` (`Auto git push 2026-06-07T07:30:09Z`)\n- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`\n- Reviewed workspace memory/log paths showed no live env-var secret exposures across 541 checked files\n- WhatsApp allowlist verified on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`\n- Process list reviewed; only expected core services observed (`node src/server.js`, `openclaw`, `openclaw-gateway`, `python3 welly-daemon.py start`) plus the review commands themselves\n- Disk usage normal (`/` 49%, `/data` 55%); no storage pressure observed\n- `openclaw security audit --deep`: `0 critical · 0 warn · 1 info` (informational only: top-level WhatsApp group allowlist empty, so non-allowlisted group messages are dropped)\n- `openclaw update status`: update available (`2026.6.1`), treated as maintenance, not an active security issue\n- Security review passed — all clear\n\n2026-06-09T06:00:31.[REDACTED_CLIENT_ID]: Auto-redacted 7 exposed credentials from files\n\n## 2026-06-09 — Nightly Security Review (02:00 America/New_York)\n- Auto-redaction ran first and removed 7 exposed credentials from `memory/security-log.md`, `memory/2026-06-08.md`, and `.git/logs/HEAD`; those fixes were auto-remediated and not escalated per policy\n- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live secret exposures after redaction; remaining hits were prior redacted security-log notes, one learning-log mention, dependency text, and one Playwright prompt artifact\n- Focused hardcoded-credential scan across `/data/workspace` found no live hardcoded credentials; remaining matches were documentation/examples, a Slack placeholder (`xoxb-...`) in `src/server.js`, and prior redacted log entries\n- Git review (last 24h): expected commit activity only — `6ed32e6` (`Auto git push 2026-06-08T07:30:16Z`)\n- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`\n- WhatsApp allowlist verified on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`; top-level WhatsApp `dmPolicy` remains `pairing`\n- Process list reviewed; only expected core services observed (`node src/server.js`, `openclaw`, `openclaw-gateway`, `python3 welly-daemon.py start`) plus the review commands themselves\n- Disk usage normal (`/` 48%, `/data` 55%); no storage pressure observed\n- `openclaw security audit --deep`: `0 critical · 0 warn · 1 info` (informational only: top-level WhatsApp group allowlist empty, so non-allowlisted group messages are dropped)\n- `openclaw update status`: update available (`2026.6.1`), treated as maintenance, not an active security incident\n- **Alertable issue found:** live secrets are still present outside `/data/workspace` in `/data/.clawdbot/openclaw.env` and multiple `openclaw.json` backup files (`openclaw.json.backup`, `openclaw.json.bak`, `openclaw.json.bak.1`–`.4`, and `openclaw.json.backup-[REDACTED_CLIENT_ID]-*`); Kelly notified via WhatsApp per policy\n\n2026-06-10T06:00:26.[REDACTED_CLIENT_ID]: Auto-redacted 7 exposed credentials from files\n
## 2026-06-10 — Nightly Security Review (02:00 America/New_York)
- Auto-redaction ran first and removed 7 exposed credentials from `memory/security-log.md` and `.git/logs/HEAD`; those fixes were auto-remediated and not escalated per policy
- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live secret exposures after redaction; remaining hits were prior redacted security-log notes, one learning-log mention, dependency text, and one Playwright prompt artifact
- Focused hardcoded-credential scan across `/data/workspace` found no live hardcoded credentials; remaining matches were documentation/examples, normal code variable assignments, and prior redacted log entries
- Git review (last 24h): expected commit activity only — `dc[REDACTED_CLIENT_ID]` (`Auto git push 2026-06-09T07:30:14Z`)
- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`
- Reviewed workspace memory/log paths showed no live env-var secret exposures; remaining matches were skill setup examples/placeholders only
- WhatsApp allowlist verified on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`; top-level WhatsApp `dmPolicy` remains `pairing`
- Process list reviewed; only expected core services observed (`node src/server.js`, `openclaw`, `openclaw-gateway`, `python3 welly-daemon.py start`) plus the review commands themselves
- Disk usage normal (`/` 48%, `/data` 55%); no storage pressure observed
- `openclaw security audit --deep`: `0 critical · 0 warn · 1 info` (informational only: top-level WhatsApp group allowlist is empty, so non-allowlisted group messages are dropped)
- `openclaw update status`: update available (`2026.6.5`), treated as maintenance, not an active security incident
- Security review passed — all clear
\n2026-06-11T06:00:55.[REDACTED_CLIENT_ID]: Auto-redacted 12 exposed credentials from files\n
## 2026-06-11 — Nightly Security Review (02:00 America/New_York)
- Auto-redaction ran first and removed 12 exposed credentials from `memory/security-log.md`; those fixes were auto-remediated and not escalated per policy
- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live secret exposures after redaction; remaining hits were prior redacted security-log notes only
- Focused hardcoded-credential scan across `/data/workspace` found no live hardcoded credentials; remaining matches were documentation/examples (`skills/youtube/setup.md`) and a Slack placeholder (`xoxb-...`) in `src/server.js`, not secrets
- Git review (last 24h): expected commit activity only — `38c56ad` (`Auto git push workspace 2026-06-10 07:30 UTC`)
- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`
- Reviewed workspace memory/log paths showed no live env-var secret exposures; remaining matches were env-var names and setup examples only
- WhatsApp allowlist verified under `channels.whatsapp.accounts` on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`; top-level WhatsApp `dmPolicy` remains `pairing`
- Process list reviewed; only expected core services observed plus the review commands themselves
- Disk usage normal (`/` 49%, `/data` 55%); no storage pressure observed
- Security review passed — all clear
\n2026-06-12T06:00:20.[REDACTED_CLIENT_ID]: Auto-redacted 13 exposed credentials from files\n\n## 2026-06-12 — Nightly Security Review (02:00 America/New_York)\n- Auto-redaction ran first and removed 13 exposed credentials from `memory/security-log.md`, `memory/2026-06-11.md`, and `.git/logs/HEAD`; those fixes were auto-remediated and not escalated per policy\n- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live secret exposures after redaction; remaining hits were prior redacted review notes, one learning-log mention, dependency text, and one Playwright prompt artifact\n- Focused hardcoded-credential scan across `/data/workspace` found no live hardcoded credentials; remaining matches were documentation/setup examples, normal variable assignments, and the Slack placeholder in `src/server.js`, not secrets\n- Git review (last 24h): expected commit activity only — `6cf7fba` (`Improve proactive presence triggers`), `c[REDACTED_CLIENT_ID]d` (`Build proactive presence evaluator`), and `1bc222c` (`Auto git push 2026-06-11T07:30:14Z`)\n- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`\n- Reviewed workspace memory/log paths showed no live env-var secret exposures after redaction; remaining matches were examples/placeholders only\n- WhatsApp allowlist verified under `channels.whatsapp.accounts` on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`; top-level WhatsApp `dmPolicy` remains `pairing`\n- Process list reviewed; only expected core services observed (`node src/server.js`, `openclaw`, `openclaw-gateway`, `python3 welly-daemon.py start`) plus the review commands themselves\n- Disk usage normal (`/` 50%, `/data` 55%); no storage pressure observed\n- **Alertable issue found outside `/data/workspace`:** `/data/.clawdbot/openclaw.env` and `openclaw.json.bak` / `openclaw.json.bak.1`–`.4` still match secret-pattern checks, indicating live secrets remain stored outside the workspace scope; Kelly notified via WhatsApp per policy\n- Gap: `openclaw security audit --deep` could not run because the local CLI still fails with missing module `@mariozechner/pi-ai/oauth`; gateway remained running, but the CLI audit itself was unavailable during this review\n\n2026-06-13T06:00:21.[REDACTED_CLIENT_ID]: Auto-redacted 341 exposed credentials from files\n
## 2026-06-13 — Nightly Security Review (02:00 America/New_York)
- Auto-redaction ran first and removed 341 exposed credentials across 143 files (`memory/security-log.md`, `.git/logs/HEAD`, and `/tmp/openclaw-gateway-manual.log` were explicitly reported); exposure fixes were auto-remediated and not escalated per policy
- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live workspace secret exposures after redaction; remaining hits were prior redacted notes, dependency text, one learning-log mention, and one Playwright prompt artifact
- Focused hardcoded-credential scan across `/data/workspace` found no live hardcoded credentials; remaining matches were documentation/setup examples, a historical redacted memory entry, and prior redacted security-log notes
- Git review (last 24h): expected commit activity only — `bf3a645` (`Rebalance proactive heartbeat messaging`) and `5fe850f` (`Auto git push`)
- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`
- Reviewed workspace memory/log paths showed no live env-var secret exposures after redaction; remaining hits were env-var names and placeholder examples only
- WhatsApp allowlist verified under `channels.whatsapp.accounts` on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`; top-level WhatsApp `dmPolicy` remains `pairing`
- Process list reviewed; only expected core services observed (`node src/server.js`, `openclaw`, `openclaw-gateway`, `python3 welly-daemon.py start`) plus the review commands themselves
- Disk usage normal (`/` 47%, `/data` 55%); no storage pressure observed
- `openclaw security audit --deep`: no critical findings; expected doctor warnings note empty top-level WhatsApp/Telegram group allowlists, which means non-allowlisted group messages are dropped
- `openclaw update status`: update available (`2026.6.6`), treated as maintenance, not an active security issue
- Note: secret-pattern matches still exist outside `/data/workspace` in `/data/.clawdbot/openclaw.env` and multiple `openclaw.json*` backup files; this is a credential-storage issue, not an additional non-credential alert, so it was logged here and not escalated via WhatsApp under tonight's rule
\n2026-06-14T06:00:08.[REDACTED_CLIENT_ID]: Auto-redacted 5 exposed credentials from files\n\n## 2026-06-14 — Nightly Security Review (02:00 America/New_York)\n- Auto-redaction ran first and removed 5 exposed credentials from files; those fixes were auto-remediated and not escalated per policy\n- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live workspace secret exposures after redaction; remaining hits were prior redacted security-log notes, dependency text in `node_modules`, and grep noise from missing skill symlink targets\n- Focused hardcoded-credential scans across `/data/workspace` found no live hardcoded credentials; remaining matches were env-var names, documentation/setup examples, or known placeholders only\n- Git review (last 24h): expected commit activity only — `[REDACTED_CLIENT_ID]` (`Auto git push 2026-06-13 07:30:06 UTC`)\n- `/data/.clawdbot/openclaw.json` permissions verified: `600 root:root`\n- Reviewed workspace memory/log paths showed no live env-var secret exposures after redaction\n- WhatsApp allowlist verified under `channels.whatsapp.accounts` on both `custom-1` and `default`: `+[REDACTED_CLIENT_ID]401` only, with `dmPolicy=allowlist` and `groupPolicy=allowlist`; top-level WhatsApp `dmPolicy` remains `pairing`\n- Process list reviewed; only expected core services observed (`node src/server.js`, `openclaw-gateway`, `python3 welly-daemon.py start`) plus the review commands themselves\n- Disk usage normal (`/` 48%, `/data` 55%); no storage pressure observed\n- Security review passed — all clear\n\n2026-06-15T06:00:08.[REDACTED_CLIENT_ID]: Auto-redacted 9 exposed credentials from files\n
## 2026-06-15 — Nightly Security Review (02:00 America/New_York)
- Auto-redaction ran first and removed 9 exposed credentials from files; those fixes were auto-remediated and not escalated per policy
- Markdown/text/json `sk-` scan in `/data/workspace` found no remaining live workspace secret exposures after redaction; remaining hits were prior redacted security-log notes, one historical memory note, documentation/setup examples, and env-var name references only
